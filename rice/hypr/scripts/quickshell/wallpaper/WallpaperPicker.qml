...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    echo "$DEST_FILE" > "$HOME/.config/hypr/wallpaper"
        ...
        if (isVideo) {
            ...
            wallpaperCmd = `
                ...
                echo "${escOriginal}" > "$HOME/.config/hypr/wallpaper"
            `;
        }
        ...
        if (closeCallback) Qt.callLater(closeCallback);
    }
    ...
    Shortcut { sequence: "Escape"; enabled: !window.isApplying; onActivated: { if (window.currentFilter === "Search") { window.currentFilter = "All"; } else { if (closeCallback) closeCallback(); } } }
