...
compile_settings() {
    ...
    INPUT_SENSITIVITY=$(jq -r '.inputSensitivity // "-0.1"' "$SETTINGS_FILE")
    ...
    install -m 644 "$TMPL_DIR/keybinds.conf.template" "$KEYBINDS_CONF"
    ...
    install -m 644 "$TMPL_DIR/monitors.conf.template" "$MONITORS_CONF"
    ...
    if [[ "$MONITOR_COUNT" -gt 0 ]]; then
        ...
        # Fallback so any unrecognised monitor still gets configured
        echo "monitor = , preferred, auto, 1" >> "$MONITORS_CONF"
        # Workspace bindings for primary monitor
        PRIMARY_MONITOR=$(jq -r '(.monitors // [])[] | select(.primary == true) | .name' "$SETTINGS_FILE" 2>/dev/null | head -1)
        if [[ -n "$PRIMARY_MONITOR" ]]; then
            for i in $(seq 1 10); do
                echo "workspace = $i, monitor:$PRIMARY_MONITOR" >> "$MONITORS_CONF"
            done
        fi
    else
        echo "monitor = , preferred, auto, 1" >> "$MONITORS_CONF"
